...
describe('OrganizeView through ManagerApp', () => {
  it('shows real exact duplicates with Chinese evidence and actionable selection controls', async () => {
    ...
    expect(screen.queryByRole('button', { name: '移动选中项' })).toBeNull();
    expect(screen.queryByRole('button', { name: '删除选中项' })).toBeNull();
    expect(screen.getAllByRole('button', { name: /编辑 Shared Copy/ })).not.toHaveLength(0);
    expect(screen.getAllByRole('button', { name: /移动 Shared Copy/ })).not.toHaveLength(0);
    expect(screen.getAllByRole('button', { name: /删除 Shared Copy/ })).not.toHaveLength(0);
    expect(screen.queryByText('English detail must stay hidden')).toBeNull();
  });

  it('uses direct row actions instead of group selection actions', async () => {
    const repository = repositoryStub(vi.fn().mockResolvedValue(organizeTree()));
    render(<ManagerApp openUrl={vi.fn()} repository={repository} />);
    await screen.findByRole('heading', { name: '书签栏' });
    fireEvent.click(screen.getByRole('button', { name: '整理' }));
    ...
    fireEvent.click(screen.getAllByRole('button', { name: '移动 Shared Copy' })[0]);
