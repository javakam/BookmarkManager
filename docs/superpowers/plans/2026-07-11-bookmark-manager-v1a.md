...
> **文档状态（2026-08-09）：** 本文件是早期实现计划和历史记录，不是现行验收清单。v1.0.4 已发布；当前工作树为已验证、已打包、待提交推送的 v1.0.5。涉及新标签页覆盖、链接检测、可选主机权限、`待删除（书签工作台）`、隔离、恢复、恢复锚点和操作记录的任务均未进入正式版本；当前删除为原生书签永久删除，并使用完整递归快照和执行前重读防止误删。当前功能、权限和测试边界以源码、构建 Manifest、自动化测试以及同目录产品设计文档顶部的版本边界为准。
...
## v1.0.5 发布批次（已验证、已打包，待提交推送）

- [x] 在原有跟随系统、浅色、深色主题上增加暖红、暖红深色和雾蓝主题，使用可预览的单选色板并继续保存到 `storage.local`。
- [x] 统一主按钮前景色变量，保证新增浅色和深色配色的文字对比；主题切换状态即时且唯一。
- [x] 搜索查询只规范化一次，并使用 React 延迟值保持输入响应。
- [x] 整理分析分两阶段在浏览器空闲时执行；离开整理页立即取消未完成阶段，短时复用后释放缓存。
- [x] 相似项和镜像目录结果分别限制为 2000 和 500 条高优先级结果，界面继续明确显示截断状态。
- [x] 增加 5000 条搜索和 7001 节点整理的时间、保留堆内存基准；补齐主题持久化、唯一选中和缩放布局 E2E。
